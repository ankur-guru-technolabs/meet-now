...
class CustomerController extends BaseController
{
    ...
    public function getProfile(Request $request){
        try{ 
            ...
            $data['user']   =  User::with('media','activeSubscription')->find($id)->except(['facebook_id', 'google_id']);
            if( !isset($request->id)){
    ...
    public function singleVideoCall(Request $request){
        try{
            ...
            if (Auth::user()) { 
                ...
                $appCertificate = env("AGORA_APP_CERTIFICATE", "3038f7b0ec7a435ebbb0885867733f25");
